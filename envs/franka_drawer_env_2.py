...
class FrankaDrawerEnv_2(gym.Env, EzPickle):
    """
    ## Description
    ...
    * `achieved_goal`: this key represents the current state of the block, as if it would have achieved a goal. This is useful for goal orientated learning algorithms such as those that use [Hindsight Experience Replay](https://arxiv.org/abs/1707.01495) (HER). The value is an `ndarray` with shape `(3,)`. The elements of the array are the following:
    ...
    def __init__(
        ...
        if control_mode in ['position', 'position_rotation']:
            self.fullpath = os.path.join(os.path.dirname(__file__), "assets", "franka_drawer_2.xml")
        else:
            self.fullpath = os.path.join(os.path.dirname(__file__), "assets", "franka_drawer_2_ik.xml")
        ...
        self.initial_qpos = {
            'object0:joint': [1.25, 0.53, 0.6, 1., 0., 0., 0.],#[1.25, 0.53, 0.4, 1., 0., 0., 0.]
            'robot:joint1': 0.0254,
            'robot:joint2': -0.2188,
            'robot:joint3': -0.0265,
            'robot:joint4': -2.6851,
    ...
    def _check_collisions(self):
        for i in range(self.data.ncon):
            ...
            for obst_id in self.obstacle_ids:
                ...
                skip = [table_id, panda_table_id, object_id, obst6]
