...
class Sim2RealEnvFranky(gym.Env, EzPickle):
    ...
    def __init__(
        ...
        self.avoid_col_init = [0., 0.3, 0., -1.2, 0., 1.5, 0., 0.02]
        self.initial_qpos = [0.0254, -0.2188, -0.0265, -2.6851, -0.0092, 2.4664, 0.0068, 0.02]
        self.obstacle_info = []
    ...
    def _get_obs(self):
        ...
        if np.array(gripper_placement) <= -0.03:
            object_pos = grip_pos
            object_velp = grip_velp
        else:
            object_pos = body_pos['object'] + np.array([0.01, 0.07, 0.01])
            object_velp = (object_pos - self.last_object_pos) / self.dt
        ...
        # Obstacle 0
        pos = [np.array(body_pos['obst1'][0]), 0.84, 0.485] + np.array([-0.015, 0, 0])
        ...
        # Obstacle 2
        pos = [np.array(body_pos['obst2'][0]), 0.64, 0.5] + np.array([-0.015, 0, 0])
